...
export default function Contact() {
  ...
  const submit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const productSelections = products.filter((product) => product.name && product.sku).map((product) => ({ name: product.name, sku: product.sku }));
    ...
    try {
      const response = await fetch(`${inquiryApiUrl}/api/public/inquiries`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name, email, country, company: company || null, inquiryType, products: productLines, productSelections, requirement: structuredRequirement, consent: true }) });
