...
export default function Feedback({ titleOverride }: { titleOverride?: string } = {}) {
  const copy = useSiteCopy();
  return <SiteLayout><main><PageHero index="04" kicker={copy.get("feedback.hero.kicker", "CUSTOMER FEEDBACK / FIELD NOTES")} title={titleOverride ? <>{titleOverride}</> : <>{copy.get("feedback.hero.title", "Customer Feedback,")}<br /><em>{copy.get("feedback.hero.titleEm", "Complete Archive.")}</em></>} intro={copy.get("feedback.hero.intro", "Your customer feedback and delivery records are shown independently in full. Every image keeps its original proportions; click to view the original image.")} /><section className="section feedback-page-section"><div className="container"><div className="feedback-top-note"><div className="quote-mark">“</div><div><p>{copy.get("feedback.note.body", "Consistent quality, clear documentation, and traceable delivery are the strongest language of long-term collaboration.")}</p><span>{copy.get("feedback.note.caption", "— TIDE / PARTNERSHIP NOTE 04")}</span></div><Link className="text-link" href="/contact">{copy.get("feedback.note.cta", "Start the Next Collaboration")} <ArrowUpRight size={15} /></Link></div><ArchiveGrid title={copy.get("feedback.archive.feedbackTitle", "Customer Feedback Files")} label={copy.get("feedback.archive.feedbackLabel", "CUSTOMER FEEDBACK / 09 FILES")} files={feedbackFiles} prefix={copy.get("feedback.archive.feedbackPrefix", "FEEDBACK FILE")} /><ArchiveGrid title={copy.get("feedback.archive.deliveryTitle", "Delivery Records")} label={copy.get("feedback.archive.deliveryLabel", "DELIVERY RECORDS / 09 FILES")} files={deliveryFiles} prefix={copy.get("feedback.archive.deliveryPrefix", "DELIVERY FILE")} /><div className="feedback-end-note"><ChevronDown size={18} /><span>{copy.get("feedback.archive.endNote", "All customer files are shown above.")}</span></div></div></section></main></SiteLayout>;
}
