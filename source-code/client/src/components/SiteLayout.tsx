...
function BrandReveal() {
  ...
  return <div className="brand-reveal" role="status" aria-label="Tide logo reveal loading" onClick={() => setVisible(false)}><div className="brand-reveal-field" aria-hidden="true"><span className="brand-drop" /><span className="brand-ripple brand-ripple-a" /><span className="brand-ripple brand-ripple-b" /><span className="brand-ripple brand-ripple-c" /></div><div className="brand-reveal-mark"><img src="/W/assets/tide-logo_e4a10c2a.png" alt="Tide scientific institution" /><span>TIDE / PEPTIDES</span></div><button className="brand-reveal-skip" type="button" onClick={(event) => { event.stopPropagation(); setVisible(false); }}>{copy.get("layout.revealSkip", "Skip")}</button></div>;
}
...
export default function SiteLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => { const items = Array.from(document.querySelectorAll<HTMLElement>(".reveal-on-scroll, .site-shell main > .section, .site-shell main > .contact-page-section")); const frame = requestAnimationFrame(() => { items.forEach((item) => item.classList.add("is-visible")); }); return () => cancelAnimationFrame(frame); }, []);
  return <div className="site-shell"><BrandReveal /><header className={`site-header page-header ${scrolled ? "is-scrolled" : ""}`}><Link className="brand" href="/" aria-label="Tide Peptides home"><img className="brand-logo" src="/W/assets/tide-logo_e4a10c2a.png" alt="Tide Peptides" /></Link><nav className="desktop-nav" aria-label="Primary navigation"><NavigationLinks /></nav><div className="header-actions"><a className="header-cta" href="https://wa.me/85253929189" target="_blank" rel="noreferrer">Request Partnership <ArrowUpRight size={15} /></a></div><button className="mobile-menu" onClick={() => setMobileOpen(!mobileOpen)} aria-label="Open menu">{mobileOpen ? <X /> : <Menu />}</button>{mobileOpen && <nav className="mobile-nav" aria-label="Mobile navigation"><NavigationLinks mobile onNavigate={() => setMobileOpen(false)} /></nav>}</header>{children}<footer className="site-footer"><div className="container footer-inner"><div className="brand footer-brand"><img className="brand-logo" src="/W/assets/tide-logo_e4a10c2a.png" alt="Tide Peptides" /></div><span>{copy.get("layout.copyright", "© 2026 Tide Peptides")}</span><span>{copy.get("layout.footerTagline", "Pure · Potent · Precise · Serving Global Research")}</span></div></footer></div>;
}
